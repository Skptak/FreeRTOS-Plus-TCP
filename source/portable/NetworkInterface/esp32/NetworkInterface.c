...
static BaseType_t xESP32_Eth_NetworkInterfaceInitialise(
    NetworkInterface_t * pxInterface );

static BaseType_t xESP32_Eth_NetworkInterfaceOutput(
    NetworkInterface_t * pxInterface,
    NetworkBufferDescriptor_t * const pxDescriptor,
    BaseType_t xReleaseAfterSend );

static BaseType_t xESP32_Eth_GetPhyLinkStatus(
    NetworkInterface_t * pxInterface );

NetworkInterface_t * pxESP32_Eth_FillInterfaceDescriptor(
    ...
    NetworkInterface_t * pxInterface )
{
    ...
    /* This function pxESP32_Eth_FillInterfaceDescriptor() adds a
    ...
    BaseType_t xReleaseAfterSend )
{
    ...
    {
        ret = esp_wifi_internal_tx( ESP_IF_WIFI_STA,
        ...
        {
            ESP_LOGE( TAG,
                      "Failed to tx buffer %p, len %d, err %d",
                      pxNetworkBuffer->pucEthernetBuffer,
                      pxNetworkBuffer->xDataLength,
                      ret );
        }
...
esp_err_t wlanif_input( void * netif, void * buffer, uint16_t len, void * eb )
{
    ...
    pxNetworkBuffer = pxGetNetworkBufferWithDescriptor( len,
                                                        xDescriptorWaitTime );
    ...
    if( pxNetworkBuffer != NULL )
    {
        ...
        {
            ESP_LOGE( TAG,
                      "Failed to enqueue packet to network stack %p, len %d",
                      buffer,
                      len );
            vReleaseNetworkBufferAndDescriptor( pxNetworkBuffer );
            return ESP_FAIL;
