...
{
    NetworkEndPoint_t * pxReturn = ( NetworkEndPoint_t * ) safeMalloc(
...
void harness()
{
    NetworkBufferDescriptor_t * const pxNetworkBuffer = safeMalloc(
        sizeof( NetworkBufferDescriptor_t ) );
    uint8_t * pucEthernetBuffer = ( uint8_t * ) safeMalloc(
        ipTOTAL_ETHERNET_FRAME_SIZE + ipIP_TYPE_OFFSET );
    ...
    /* Points to ethernet buffer offset by ipIP_TYPE_OFFSET, this make sure the
     * buffer allocation is similar to the pxGetNetworkBufferWithDescriptor */
    pxNetworkBuffer->pucEthernetBuffer = &(
        pucEthernetBuffer[ ipIP_TYPE_OFFSET ] );
    __CPROVER_assume( pxNetworkBuffer->pucEthernetBuffer != NULL );

    /* Minimum length of the pxNetworkBuffer->xDataLength is at least the size
     * of the IPPacket_IPv6_t. */
    __CPROVER_assume(
        ( pxNetworkBuffer->xDataLength >= sizeof( IPPacket_IPv6_t ) ) &&
    ...
    IPPacket_IPv6_t * const pxIPPacket = ( IPPacket_IPv6_t * )
                                             pxNetworkBuffer->pucEthernetBuffer;

    prvAllowIPPacketIPv6( &pxIPPacket->xIPHeader,
                          pxNetworkBuffer,
