...
/* This proof assumes pxTCPSocketLookup, vTCPStateChange, prvTCPSocketIsActive,
 * xIsCallingFromIPTask, xSequenceGreaterThan, xSequenceLessThan,
 * xTaskGetTickCount, vReleaseNetworkBufferAndDescriptor, xTCPWindowTxHasData
 * and pxGetNetworkBufferWithDescriptor are implemented correctly.
 *
 * It also assumes prvSingleStepTCPHeaderOptions, prvCheckOptions,
 * prvTCPPrepareSend, prvTCPHandleState, prvHandleListen_IPV6 and
...
/* Abstraction of uxIPHeaderSizePacket. Because we're testing IPv6 in this test
 * case, the network buffer is guaranteed to be IPv6 packet. Thus returns IPv6
 * header size here directly. */
...
/* Abstraction of uxIPHeaderSizePacket. Because we're testing IPv6 in this test
 * case, all socket handlers returned by functions are for IPv6. Thus returns
 * IPv6 header size here directly. */
...
void harness()
{
    ...
    pxNetworkBuffer->pucEthernetBuffer = safeMalloc(
        sizeof( TCPPacket_IPv6_t ) );

    /* To avoid asserting on the ethernet buffer being NULL. */
    __CPROVER_assume( pxNetworkBuffer->pucEthernetBuffer != NULL );

    /* Ethernet frame type is checked before calling
     * xProcessReceivedTCPPacket_IPV6. */
