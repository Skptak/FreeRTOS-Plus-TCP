...
int32_t __CPROVER_file_local_FreeRTOS_TCP_IP_c_prvSingleStepTCPHeaderOptions(
...
int32_t __CPROVER_file_local_FreeRTOS_TCP_Reception_c_prvSingleStepTCPHeaderOptions(
    const uint8_t * const pucPtr,
    size_t uxTotalLength,
    FreeRTOS_Socket_t * const pxSocket,
    BaseType_t xHasSYNFlag )
{
    ...
    __CPROVER_assert( buffer_size < CBMC_MAX_OBJECT_SIZE,
                      "prvSingleStepTCPHeaderOptions: buffer_size < "
    ...
    __CPROVER_assert( uxTotalLength <= buffer_size,
                      "prvSingleStepTCPHeaderOptions: uxTotalLength <= "
                      "buffer_size" );
    ...
    __CPROVER_assume( ( index == -1 ) || ( index == 1 ) ||
                      ( index <= uxTotalLength ) );
