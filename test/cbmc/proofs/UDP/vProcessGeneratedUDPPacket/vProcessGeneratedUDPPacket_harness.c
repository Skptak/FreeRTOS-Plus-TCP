    ...
    NetworkBufferDescriptor_t * const pxNetworkBuffer )
{
    __CPROVER_assert( pxNetworkBuffer != NULL,
                      "The network buffer cannot be NULL." );
}
...
void harness()
{
    ...
    /* Assume that the size of packet must be greater than that of UDP-Packet
     * and less than that of the Ethernet Frame Size. */
    __CPROVER_assume( xRequestedSizeBytes >= sizeof( UDPPacket_t ) &&
    ...
    NetworkBufferDescriptor_t * const
        pxNetworkBuffer = pxGetNetworkBufferWithDescriptor( xRequestedSizeBytes,
                                                            0 );
    ...
    pxNetworkEndPoints->pxNetworkInterface = pxNetworkBuffer->pxEndPoint
                                                 ->pxNetworkInterface;
    ...
    if( nondet_bool() )
    {
        ...
        pxNetworkEndPoints->pxNext
            ->pxNetworkInterface = pxNetworkBuffer->pxEndPoint
                                       ->pxNetworkInterface;
