...
{
    NetworkEndPoint_t * pxReturnEndPoint = ( NetworkEndPoint_t * ) safeMalloc(
...
NetworkBufferDescriptor_t * pxDuplicateNetworkBufferWithDescriptor(
    ...
    size_t uxNewLength )
{
    /* In this function, we can either return NULL or valid pointer as return
     * value. */
    NetworkBufferDescriptor_t *
        pxReturnNetworkBuffer = ( NetworkBufferDescriptor_t * ) safeMalloc(
            sizeof( NetworkBufferDescriptor_t ) );

    if( pxReturnNetworkBuffer )
    {
        /* If the pointer is valid, we must have enough ethernet buffer for it
...
void harness()
{
    ...
    if( pxNetworkBuffer )
    {
        /* If buffer pointer is valid, allocate ethernet buffer and set data
         * length for it. */
