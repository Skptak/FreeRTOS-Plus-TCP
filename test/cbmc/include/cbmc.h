...
#define __CPROVER_printf( var )                    \
    {                                              \
        uint32_t ValueOf_##var = ( uint32_t ) var; \
    }
#define __CPROVER_printf2( str, exp )                  \
    {                                                  \
        uint32_t ValueOf_##str = ( uint32_t ) ( exp ); \
    }

/* CBMC debugging: printfs for pointer expressions */

#define __CPROVER_printf_ptr( var )                  \
    {                                                \
        uint8_t * ValueOf_##var = ( uint8_t * ) var; \
    }
#define __CPROVER_printf2_ptr( str, exp )                \
    {                                                    \
        uint8_t * ValueOf_##str = ( uint8_t * ) ( exp ); \
    }
